...
def _test_cd_pipe(comment_file_path, student_dir, command_wait=0.01):
...
def _test_cat_echo(comment_file_path, student_dir, command_wait=0.01):
...
def _test_cat_wc(comment_file_path, student_dir, command_wait=0.01):
...
def _test_variable_echo(comment_file_path, student_dir, command_wait=0.01):
    ...
    try:
        ...
        write(p,"x=5 | echo $x")
        output = read_stdout(p)
        if "5" in output or "$x" not in output:
            finish(comment_file_path, "NOT OK")
            return
...
def _test_redefine_echo(comment_file_path, student_dir, command_wait=0.01):
...
# Pipe Error Handling
def _test_long_line(comment_file_path, student_dir, command_wait=0.01):
    start_test(comment_file_path, "Pipe line cannot exceed the character limit")

    try:
        p = start('./mysh')
        write(p,"echo bigword1 | echo bigword2 | echo bigword3 | echo bigword4 | echo bigword5")
...
def _test_error_chain(comment_file_path, student_dir, command_wait=0.01):
...
def _test_no_space(comment_file_path, student_dir, command_wait=0.01):
...
def _test_non_existant(comment_file_path, student_dir, command_wait=0.01):
    start_test(comment_file_path, "Pipe to a command that does not exist reports uncrecognized error")

    try:
        p = start('./mysh')
        write(p,"echo sample | fakecommand")
        output1 = read_stderr(p)
        if "ERROR: Unrecognized command: fakecommand" not in output1:
            finish(comment_file_path, "NOT OK")
            return
...
def _test_cat_cat_wc(comment_file_path, student_dir, command_wait=0.01):
    ...
    try:
        p = start('./mysh')
        write(p,"cat testfile.txt | cat testfile.txt | wc")
        output1 = read_stdout(p)
        if "word count" not in output1:
            finish(comment_file_path, "NOT OK")
            return
...
def _test_echo_cat_wc(comment_file_path, student_dir, command_wait=0.01):
...
def _test_cat_cat_wc2(comment_file_path, student_dir, command_wait=0.01):
    start_test(comment_file_path, "Cat builtin supports nested multi pipes")
    fptr = open(student_dir + "/testfile.txt", "w")
    fptr.write("word1 word2 word3\n")
    fptr.close()
    ...
    try:
        p = start('./mysh')
        write(p,"cat testfile.txt | cat testfile.txt | wc | wc")
        output1 = read_stdout(p)
        if "word count" not in output1:
            finish(comment_file_path, "NOT OK")
            return
...
def _test_echo_cat_wc2(comment_file_path, student_dir, command_wait=0.01):
    start_test(comment_file_path, "Multi-level pipe with echo, cat, and wc")
    try:
        p = start('./mysh')
        write(p,"echo a | cat | wc | wc | wc | wc | wc | wc")
        output1 = read_stdout(p)
        if "word count 9" not in output1:
            finish(comment_file_path, "NOT OK" + output1)
            return
        output2 = read_stdout(p)
        if "character count 48" not in output2:
            finish(comment_file_path, "NOT OK")
            return
        output3 = read_stdout(p)
        if "newline count 3" not in output3:
            finish(comment_file_path, "NOT OK")
            return
        finish(comment_file_path, "OK")
    except Exception as e:
...
def test_builtin_pipes_suite(comment_file_path, student_dir):
    ...
    start_suite(comment_file_path, "Multi Level pipes")
    start_with_timeout(_test_cat_cat_wc, comment_file_path, student_dir)
    start_with_timeout(_test_echo_cat_wc2, comment_file_path, student_dir)
    end_suite(comment_file_path)
